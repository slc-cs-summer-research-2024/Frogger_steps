...
def game():
    ...
    water =pygame.Rect(0, 50, screenW, 195)
    log_length = 600/5
    log_height = 45
    ...
    while running:
        ...
        redraw(win, frog_x, frog_y, frog_size,trucks, truck_length, truck_height,logs,log_length,log_height,water)
        if frog_y == 0:
            win_game()
    pygame.quit()
...
def move(keys, frog_x, frog_y, frog_speed, screenW, screenH, frog_size,logs,log_length,log_height):
    ...
    for log in logs:
        ...
        if log_rect.colliderect(pygame.Rect(frog_x, frog_y, frog_size, frog_size)):
            onLog = True
            frog_x += log['dir']
    if  50 <= frog_y <= 195 and not onLog:
        death()
    return frog_x, frog_y


def redraw(win, frog_x, frog_y, frog_size,trucks, truck_length, truck_height,logs,log_length,log_height,water):
    win.fill("black")
